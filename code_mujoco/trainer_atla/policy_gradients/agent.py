...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        if not self.CPU:
            ...
            torch.cuda.set_device(self.params.CUDA_ID)
        ...
        # Minimax training
        if self.MODE == 'adv_ppo' or self.MODE == 'adv_trpo' or self.MODE == 'adv_sa_ppo' or self.MODE == 'adv_pa_ppo':
            ...
            if not self.CPU:
                self.adversary_policy_model = self.adversary_policy_model.to("cuda:{}".format(self.params.CUDA_ID))
                self.adversary_val_model = self.adversary_val_model.to("cuda:{}".format(self.params.CUDA_ID))
    ...
    def reset_envs(self, envs):
        ...
        if self.CPU:
            ...
            return cu_tensorize([env.reset() for env in envs], self.params.CUDA_ID).unsqueeze(1)
    ...
    def multi_actor_step(self, actions, envs):
        ...
        if self.CPU:
            data = list(map(cpu_tensorize, [normed_rewards, states, not_dones]))
        else:
            data = list(map(lambda p: cu_tensorize(p, cuda_id=self.params.CUDA_ID), [normed_rewards, states, not_dones]))
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        if collect_adversary_trajectory:
            ...
            assert self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == "adv_pa_ppo"
            if self.MODE == "adv_pa_ppo":
        ...
        is_advpolicy_training = self.MODE == "adv_ppo" or self.MODE == "adv_trpo" or self.MODE == "adv_sa_ppo" or self.MODE == "adv_pa_ppo"
            ...
            if is_advpolicy_training:
                ...
                if collect_adversary_trajectory or self.params.ADV_ADVERSARY_RATIO >= random.random():
                    if self.MODE == "adv_pa_ppo":
                        ...
                        # add the perturbation to state, but use fgsm to calculate the perturbation to obs
                        next_obs_perturbations = self.perturb_obs_fgsm(next_adv_perturbations, last_states)
                        last_states = last_states + next_obs_perturbations
    ...
    def agent_from_data(store, row, cpu, extra_params=None, override_params=None, excluded_params=None):
        ...
        def load_state_dict(model, ckpt_name):
            mapper = ch.to('cuda:{}'.format(extra_params.CUDA_ID)) if not cpu else ch.device('cpu')
