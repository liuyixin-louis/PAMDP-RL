...
def main(params):
    ...
    if params['mode'] == "adv_ppo" or params['mode'] == 'adv_trpo' or params['mode'] == 'adv_sa_ppo' or params[
        'mode'] == 'adv_pa_ppo':
    ...
    # our_change: load attacker_model
    attacker = Trainer.agent_from_params(params, store=store)
    attacker.attack_policy_network.load_state_dict(params['load_attack_model'])
    p.outer_attack_model = attacker
...
def add_common_parser_opts(parser):
    ...
    parser.add_argument('--mode', type=str,
                        choices=['ppo', 'trpo', 'robust_ppo', 'adv_ppo', 'adv_trpo', 'adv_sa_ppo', 'adv_pa_ppo'],
                        help='pg alg')
    ...
    parser.add_argument('--attack-method', type=str,
                        choices=["none", "critic", "random", "action", "sarsa", "sarsa+action", "advpolicy",
                                 "paadvpolicy", "action+imit"], help='adversarial attack methods.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load-model', type=str, default=None, required=False,
                        help='load pretrained model and optimizer states before training')
    parser.add_argument('--no-load-adv-policy', action='store_true', required=False,
                        help='Do not load adversary policy and value network from pretrained model.')
    parser.add_argument('--adv-policy-only', action='store_true', required=False,
                        help='Run adversary only, by setting main agent learning rate to 0')
    parser.add_argument('--deterministic', action='store_true',
                        help='disable Gaussian noise in action for --adv-policy-only mode')
    parser.add_argument('--seed', type=int, help='random seed', default=-1)
    parser.add_argument("--load-attack-model",type=str,default=None,required=True,
                        help="load the attacker model")
    parser.add_argument("--load-attack-model",type=str,default=None,required=True,
                        help="load the attacker model")
    ...
    extra_params = ['config_path', 'out_dir_prefix', 'load_model', 'no_load_adv_policy', 'adv_policy_only',
                    'deterministic', 'seed']
